...
class DataGeneration(Geometry):
    ...
    def generate_feature_target_sf_dtr(self):
        ...
        for mol in range(len(molecule_dir)):
            ...
            for geo in dir_idx:
                lock = mp.Lock()
                mp.Process(target=self.generation_procedure_dtr,args=(lock,mol,geo)).start()
    ...
    def generation_procedure_dtr(self,mol,geom):
        #INIT GEOMETRY
        #
    ...
    def generation_procedure_gnn(self,mol,geom):
            #INIT GEOMETRY
            #
            #GENERATE TARGET & FEATURE --> picks dependend on env the right features and target
            if geom in self.comp_idx:
