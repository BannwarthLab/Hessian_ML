...
class Rotation_Functions:
    ...
    def rot_gradient(self, R):
        self.gradient = np.matmul(self.gradient,R)
    ...
    def _check_x_axis_error(self,vec_x,i,j,coord_th = 1e-7):

        if np.abs(vec_x[1]) > coord_th or np.abs(vec_x[2]) > coord_th:
            print(f"Error in vec_x for {i,j}")
            print(vec_x)

    def _check_xyz_error(self,xyz,i,j,rotation_matrix,coord_th=1e-7):
        xyz_temp_i = np.matmul(rotation_matrix, xyz[i, :])
        xyz_temp_j = np.matmul(rotation_matrix, xyz[j, :])
    ...
    def rotate_matrix(self,rotation_matrix:np.ndarray,matrix:np.ndarray)-> np.ndarray:
        return np.matmul(np.matmul(rotation_matrix,matrix),np.transpose(rotation_matrix))

    def rotate_vector_array(self,rotation_matrix:np.ndarray,vectors:np.ndarray)-> np.ndarray:
        return np.matmul(vectors,np.transpose(rotation_matrix))

    def rotate_vector(self,rotation_matrix:np.ndarray,vector:np.ndarray)-> np.ndarray:
        return np.matmul(rotation_matrix,vector)

    @checkTiming(enabled=False)
    def get_R_euler(self, coord_end:np.ndarray, dipm:np.ndarray, i:int, j:int) -> np.ndarray:
        zero_th = 0.0
        sum_th = 1e-12
        if i == j:
            print("error")
        ...
        self._check_xyz_error(coord_end,i,j,R_euler,coord_th=1e-7)

        return R_euler

    @checkTiming(enabled=False)
    def get_R_euler2(self, coord_end:np.ndarray, dipm:np.ndarray, i:int, j:int) -> np.ndarray:
        zero_th = 0.0
        sum_th = 1e-12
        if i == j:
            print("error")
        ...
        vec_dipm =  np.cross(dipm[i, :],dipm[j, :])
