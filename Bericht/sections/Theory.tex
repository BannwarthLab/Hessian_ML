...
Supervised Machine Learning models rely on the two quantities feature and target.
...
As already introduced the Hessian matrix is the target. 
...
\begin{figure}[H]
    ...
    \caption[Depiction of two Coordinate systems for description of Euler Angles.]{ Depiction of two coordinate systems  $\textbf{x},\textbf{y},\textbf{z}$ (black) and $\textbf{x}',\textbf{y}',\textbf{z}'$ (blue). 
			   The red intersection of the $\textbf{xy}$ and $\textbf{x'y'}$ planes is between the vector $\textbf{L}_\mathrm{a}$ and $\textbf{L}_\mathrm{b}$.}
...
Atomic units are used for all equations in this section. This introduction of Hartree Fock and Density Functional Theory is based on the following reference.\supercite{Jensen2013}
...
\end{align}
The determination of the total energy of the system is an eigenvalue problem and can be solved by minimization.
...
This equation is used to find a solution for the energy iteratively.
...
% From the matrix $\textbf{D}$, is then used for the calculation of the new Fock matrix.
	...
	A short introduction into this method is given in this part.
	The introduction is based on the following reference.\supercite{Bannwarth2019_GFN2xTB}.
	...
	The bonding is then described as a perturbation of this density.
...
and electrostatic energy~$E_\mathrm{ES}^{(2)}$ are described in \mbox{GFN2--xTB} with an isotropic and anisotropic contribution.
...
which leads to the complete energy equation shown in Eq.~\eqref{eq:GFN2:E}.
	...
	The Hessian is of interest, due to its need for the computation of harmonic vibrational frequencies of molecules and thermodynamic quantities.
	The harmonic vibrational frequencies can be used for the computation of wave numbers, with which IR spectra can be constructed.
	Furthermore, they can be used for the transition state search, as imaginary harmonic vibrational frequencies indicate a transition state.
	...
	of the energy in respect to every atom coordinate pair in the system.\supercite{Jensen2013}
	\begin{align}
		\textbf{H} =
		\left(\begin{matrix}
	...
	This section deals with basics of the ML algorithms and is based on the following reference.\supercite{Hastie2009}
	...
	The CART algorithm will be explained with an example. Assuming a response $Y$ that depends on two variables $X_1$ and $X_2$ is assumed.
	...
	In software like \texttt{sklearn}\supercite{scikit-learn} it is eligible to use bootstrapping.
