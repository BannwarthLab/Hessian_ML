...
Supervised ML models are based on features and the target.
Features are information given to the model, which then predicts the target value.
To train the model features and targets are computed via GFN2--xTB method.
To achieve a deliberate system a transformation of coordinates is necessary,
for which Euler angles are used.
As already introduced the Hessian matrix elements are the target. 
In the last part of this chapter,
the two ML algorithms used in this research will be discussed.
...
In the Hartee--Fock model only a single Slater determinant is used for the description of the wave function.
\begin{align}
	\label{eq:SlaterD}
...
% From the matrix $\textbf{D}$, is then used for the calculation of the new Fock matrix.
	...
	\end{align}
	In contrast to other DFTB methods, the derivation of the energy expansion for GFN2--xB includes a non--local correlation contribution from the start.
...
which leads to the complete energy equation shown in Eq.~\eqref{eq:GFN2:E}.
	...
	As for the computation of the HVF the Hessian needs to be projected and weighted, a brief introduction to this is given.\\
	...
	If the sum of the norm of two coordinates is below the threshold of $ \left\| \textbf{r}^{x,y,z} \right\| < 1 \cdot 10^{-6}$ a linear molecule is assumed.\\
	...
	Often these decision tree based algorithms are based on the Classification and Regression Tree (CART) algorithm.
 	Assuming a response $Y$ that depends on two variables $X_1$ and $X_2$ is assumed.
	The variables $X_1$ and $X_2$ are called features.
