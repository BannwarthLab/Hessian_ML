...
The acceleration of the computation is therefore of great interest.\supercite{SpicherSHP2021}\\
In general, the computation is done on a multi--level approach,
as the computation of single--point energies or optimized geometries is cheap compared to frequency calculations.
As the HVF calculation is costly, the used methods need to be cheap.
There are different approaches in literature for either accelerating the computation or to increase the precision of Hessian matrices.\\
...
This is beneficial regarding, for example, semi--empirical methods, as those depend on empirical derived data.
...
to predict the Hessian matrix by ML models to accelerate its computation.
