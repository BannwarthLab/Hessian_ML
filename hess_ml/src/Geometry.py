...
class Geometry(HessTarget,HessFeature, Input,Preparation,Observables,Rotation_Functions):
    ...
    def gen_data(self,threads):
        ...
        if os.path.isfile(os.path.join(self.folder,self.xyz_file)):
            ...
            self.xyz_pd,self.header = self.import_coord(os.path.join(self.folder,self.xyz_file))
            ...
            if self.N_atoms > 1:
                ...
                if self.do_calc:
                    ...
                    self.hessian = self.import_hessian(os.path.join(self.folder,self.target_file),self.N_atoms)
                    ...
                    np.savetxt(fname=os.path.join(self.folder,'features') ,X=self.Feature_AB)
                    np.savetxt(fname=os.path.join(self.folder,'targets') ,X=self.Target_AB)
    ...
    def hessians_difference(self,hess1,hess2):
        ...
        self.hessian1 = self.import_hessian(os.path.join(self.folder,hess1),self.N_atoms)

        self.hessian2 = self.import_hessian(os.path.join(self.folder,hess2),self.N_atoms)
