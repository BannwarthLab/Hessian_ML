        ...
        for atom_A in range(self.N_atoms):
            ...
            for atom_B in range(atom_A+1,self.N_atoms):
                ...
                if [atom_A,atom_B] in transpose_list:
                    H_APF = np.matmul(np.matmul(self.rot_X(np.pi),np.transpose(H_APF)),np.transpose(self.rot_X(np.pi)))
                    H_APF = np.matmul(np.matmul(self.rot_Z(np.pi),(H_APF)),np.transpose(self.rot_Z(np.pi)))
