...
class FeatureTBliteLegacy:
    ...
    def ImportFeature(self):
        ...
        try: 
            ...
            self.ReadGradient(os.path.join(self.folder,"gradient"))
            self.FilterFeatures()
        except:
    ...
    def FilterFeatures(self):
        ...
        for orb in ["s", "p", "d", "A", "e", "Z"]:
            ...
            if not (orb in {"e", "Z"}):
                ...
                if orb != "A":
                    self.p[f"{orb}"] = self.ml_feat.filter(regex=f"q_{orb}").to_numpy()
        ...
        self.q["default"] = self.ml_feat.loc[:, "p_A"].to_numpy()
        self.q["delta"] = self.ml_feat.loc[:, "delta_p_A"].to_numpy()
