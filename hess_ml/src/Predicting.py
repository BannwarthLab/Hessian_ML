...
class Predicting(Input,Output,Observables):
    ...
    def predict(self,files,folder=''):
        ...
        if self.config['predict'].get('normalization',False):#self.normalization

            pathname = os.path.join(folder,f'{self.model_name}_transformer.joblib')

            self.transformer = load(pathname)

            pathname = os.path.join(folder,f'{self.model_name}_transformer_target.joblib')
            
            self.target_transformer = load(pathname)


        if self.config['predict'].get('selection',False):
    ...
    def predict_hessian(self,file):
        ...
        mol = Geometry(file,self.config['geometry'])
        ...
        if mol.do_calc:
            ...
            if self.config['predict'].get('selection',False):#self.selection
                ...
                H_hetero = self.model.predict(self.selector.transform(np.array(mol.Feature_AB)))

            if self.config['predict'].get('normalization',False):

                H_hetero = self.model.predict(self.transformer.transform(np.array(mol.Feature_AB)))
