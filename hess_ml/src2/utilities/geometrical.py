...
def get_atom_pair_rot_mat_new(xyz:np.ndarray,supporting_vector:np.ndarray,indices:tuple[int,int]) -> np.ndarray:
    ##This currently does not work as expected as the wrong rotation matrix is constructed if the norm of y or x is zero. 
    i,j = indices
    xyz_i = xyz[i].copy()
    xyz_j = xyz[j].copy()
    ...
    print(i,j)
    print(supporting_vector)

    vec_x = np.cross(xyz_i, supporting_vector)
    vec_y = np.cross(xyz_i,vec_x)

    vec_x_norm = np.linalg.norm(vec_x)
    vec_y_norm = np.linalg.norm(vec_y)
    vec_z_norm = np.linalg.norm(xyz_i)

    
    if vec_x_norm < 1e-7 or vec_y_norm < 1e-7:
        print("unity matrix set end")
        R_euler = np.eye(3)
        print(np.cross(R_euler[0],np.abs(xyz_i)))
        print(np.cross(R_euler[1],np.abs(xyz_i)))
        R_euler[2] = np.abs(xyz_i)/vec_z_norm
    
    else:
        vec_x /= vec_x_norm
        vec_y /= vec_y_norm#np.linalg.norm(vec_y)
        vec_z = xyz_i/vec_z_norm#/np.linalg.norm(xyz_i)
        
        print(vec_x,"\n",vec_y,"\n",vec_z)
        print("end")

        R_euler  = np.vstack((vec_x,vec_y,vec_z))
...
def get_atom_pair_rot_mat(xyz:np.ndarray,supporting_vector:np.ndarray,indices:tuple[int,int]) -> np.ndarray:
