...
def get_feature_name(feature_col_name):
    ...
    feature_name_full = ['atom1','atom2','y_idx','y','d/nd','Rab']#,'nucA','nucB','Rab']
...
def plot_non_diag_perm_importances(model,X,y,col_name,info):
    ...
    ticks = np.array(range(N_features+1))
    fig ,ax = plt.subplots()
    ax.bar(ticks[0] ,importances[0],alpha=1.0, width=0.15, label = 'Rab')

    ax.bar(ticks[1:41]-0.3 ,importances[1:N_features+1],alpha=1.0, width=0.15, label = 'A')
    ax.bar(ticks[1:41]-0.15,importances[N_features+1:2*N_features+1],alpha=1.0, width=0.15, label = 'B')#'atom A')
    ax.bar(ticks[1:41],importances[2*N_features+1:3*N_features+1],alpha=1.0, width=0.15, label = 'arithmetic mean')#'atom B')
    ax.bar(ticks[1:41]+0.15,importances[3*N_features+1:4*N_features+1],alpha=1.0, width=0.15, label = 'product')#'arithmetic mean')
    ax.bar(ticks[1:41]+0.3 ,importances[4*N_features+1:5*N_features+1],alpha=1.0, width=0.15, label = 'absolute difference')
    ...
    ax.set_ylabel('R² coefficient')
    ax.set_xticks(ticks)
    ax.set_xticklabels(col_name[:41],rotation=90)
    plt.gcf().subplots_adjust(bottom=0.45)
    fig.set_figwidth(15)
