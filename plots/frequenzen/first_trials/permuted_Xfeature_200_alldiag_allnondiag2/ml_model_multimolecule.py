...
X_diag = np.array(FT_diag[col_name[9:50]])
y_diag = np.array(FT_diag['y'])

X_non_diag = np.array(FT_non_diag[col_name[9:]])
...
for rnd_state in [0,22,42,100,63]:#,54,96,35,408,74]:
    ...
    regr_diag =  ExtraTreesRegressor(n_estimators = 300,max_features=20,random_state=rnd_state,bootstrap=False) 

    #regr_diag = KNeighborsRegressor(n_neighbors= 25 ,weights='distance')
    #regr_diag = GridSearchCV(svm.SVR(),param_grid,cv = 2)

    regr_non_diag = ExtraTreesRegressor(n_estimators = 100,max_features=20,random_state=rnd_state,bootstrap=False)
    ...
    plot_diag_perm_importances(model_diag,X_test,y_test,col_name[9:50],rnd_state)
    ...
    plot_non_diag_perm_importances(model_non_diag,X_non_diag_test,y_non_diag_test,col_name[9:50],rnd_state)
