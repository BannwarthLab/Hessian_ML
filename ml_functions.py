    ...
    for mol in range(X_df['molecule'].max()+1):
        ...
        for var in range(var_len):
            ...
            for i in range(2):
                ...
                y_diag_prep.loc[k+i,col_name_y] = y_temp_diag.loc[i,col_name_y]

                X_diag_prep.loc[k+i,col_name] = X_temp_diag.loc[i,col_name]
...
def hess_block_to_hess_vec(hess_mat,A,B):
    ...
    for i in range(3):
        for j in range(3):
            hess_vec[k] =  hess_mat[3*int(B)+i,3*int(A)+j]
            k+=1
    return hess_vec
...
def import_files(file_path_mol):
    ...
    hessian_df.insert(0,'block',['diag','nondiag','nondiag','diag'])
    ...
    ##############################
    #Extract features
    file = ml_feature.loc[:,['index','apf','block']]
