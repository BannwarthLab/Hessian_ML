...
X_df,y_df,col_name = gen_X_y_DF(molecules)
...
gss = GroupShuffleSplit(n_splits=1,train_size=0.75,random_state = 11)
...
hess_pred = hess.copy() #matmul(matmul(np.transpose(P),hess),P)
hessian_mass = mass_weighted_hessian(hess_pred.copy(),coord['atoms'])
lamb, Q1 = linalg.eigh(hessian_mass)
...
np.savetxt('hess_xtb.txt',hess_test)

#hess_test = import_hess(f'{file_path[file_num]}'+'/init_coord/hessian',coord)

hessian_mass = mass_weighted_hessian(hess_test.copy(),coord['atoms'])
lamb, Q2 = linalg.eigh(hessian_mass)
freq1 = (np.sqrt(abs(lamb))/(atomic_time_unit*2*np.pi*speed_of_light))
